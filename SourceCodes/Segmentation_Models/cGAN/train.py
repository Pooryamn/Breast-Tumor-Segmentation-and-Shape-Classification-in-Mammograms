...
#root_path = "./dataset/"
#root_path = "/content/drive/MyDrive/Dataset/INbreast/dataset/" 
root_path = "../input/inbreast/INbreast_final/"
...
def train(epoch):
    ...
    for iteration, batch in enumerate(training_data_loader, 1):
        ...
        real_A.resize_(real_a_cpu[0].size()).copy_(real_a_cpu[0])
        real_B.resize_(real_b_cpu[0].size()).copy_(real_b_cpu[0])

        output = netD(torch.cat((real_A, real_B), 1))
        # Poorya
        #label_copy = label.clone()
        #with torch.no_grad():
        #label_copy.resize_(output.size()).fill_(real_label)
        ...
        # Poorya
        #label_copy2 = label.clone()
        #with torch.no_grad():
        #label_copy2.resize_(output.size()).fill_(fake_label)
        ...
        # Poorya
        #label_copy3 = label.clone()
        #with torch.no_grad():
        #label_copy3.resize_(output.size()).fill_(real_label)
        label.resize_(output.size()).fill_(real_label)
        ...
        D_error_epoch.append(float('{:.4f}'.format(err_d)))
        G_error_epoch.append(float('{:.4f}'.format(err_g)))

    return D_error_epoch, G_error_epoch
...
def test():
    ...
    for batch in testing_data_loader:
        input, target = Variable(batch[0][0]), Variable(batch[1][0])
        if opt.cuda:
            input = input.cuda()
            target = target.cuda()
    ...
    PSNR_R = avg_psnr / len(testing_data_loader)
    print("===> Avg. PSNR: {:.4f} dB".format(PSNR_R))
    return PSNR_R
...
def checkpoint(epoch):
    ...
    if os.path.exists("./checkpoint/{}/netD_model_epoch_{}.pth".format(opt.dataset, epoch-1)):
        os.remove("./checkpoint/{}/netD_model_epoch_{}.pth".format(opt.dataset, epoch-1))
        os.remove("./checkpoint/{}/netG_model_epoch_{}.pth".format(opt.dataset, epoch-1))
...
for epoch in range(1, opt.nEpochs + 1):
    D_error_epoch, G_error_epoch = train(epoch)

    D_error.extend(D_error_epoch)
    G_error.extend(G_error_epoch)
...
plt.legend(['G Loss'], loc='upper left')
plt.savefig('./LOSSES_D.jpg',dpi=300)
plt.show()
...
plt.ylabel('loss')
plt.xlabel('iteration')
plt.legend(['D Loss'], loc='upper left')
plt.savefig('./LOSSES_G.jpg',dpi=300)
plt.show()
